    	...
    	<div class="col-9 form-col">
            ...
            <div class="form-group">
                <div class="form-inner">
                    <h3 class="text-center">{{__('trans.Forgot password?')}}</h3>
                    <p class="text-center fs-6">{{__("trans.Enter the email address associated with your account and we'll send you a link to reset your password")}}.</p>
                    <form action="{{route('website.send_forgot_password')}}" method="POST" id="PwdRstFrmH">
                        @csrf
                        <input type="email" name="email" placeholder="{{__('trans.Email Address')}}" class="logininp @error('email') is-invalid @enderror"
                        ...
                        @enderror
                        <button class="loginbtn">{{__('trans.Reset')}}</button>
                    </form>
                </div>
            </div>
        </div>
